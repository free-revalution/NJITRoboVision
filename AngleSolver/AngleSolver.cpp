...
void AngleSolver::setCameraParam_JL(const cv::Mat &camera_matrix, const cv::Mat & distortion_coeff)
{
	camera_matrix.copyTo(CAMERA_MATRIX);
	distortion_coeff.copyTo(DISTORTION_COEFF);
}

int AngleSolver::setCameraParam_JL(const char * filePath, int camId)
{
    FileStorage fsRead;
    ...
    setCameraParam_JL(camera_matrix, distortion_coeff);
    fsRead.release();
    return 0;
}
